...
df = data(datafile)
seeing_med = df.resample('2H', on='time').median().dropna()
...
met = met.between_time('20:00','04:00')  # take only between these hours
...
#met.index.name = None
met = met.resample('2H', on='time').median().dropna()
...
border = int(0.7*len(merged))
Xtrain = merged[['temp','RH','wind_speed']].iloc[:border,:] # other 
Ytrain = merged[['seeing']].iloc[:border,:]
Xtest = merged[['temp','RH','wind_speed']].iloc[border:,:] 
Ytest = merged[['seeing']].iloc[border:,:]
lm = LinearRegression()  # create object for the class
model = lm.fit(Xtrain,Ytrain) # perform linear regression fit
model.score(Xtrain,Ytrain)
model.coef_  # list of coefficients for the slope
model.intercept_ # intercept point on y axis 
...
plt.title(name)
plt.scatter(Ytest, Ypred, s=12,c='cadetblue')
plt.plot(Ytest,Ytest, 'lightseagreen',linewidth=1.5)
